...
\begin{itemize}
    \item \textbf{Comprehensive empirical evaluation}: We present results from 360 controlled experiments across three datasets with varying complexity, using eight popular optimizers with robust statistical validation (15 seeds per configuration).
    ...
    \item \textbf{Statistical rigor}: We employ Friedman tests and pairwise comparisons to establish statistical significance of differences between optimizers, addressing a key limitation in prior work.
...
Statistical significance was assessed using non-parametric methods appropriate for our experimental design:

\begin{itemize}
    \item \textbf{Friedman test}: Overall differences between optimizers within each dataset
    \item \textbf{Wilcoxon signed-rank test}: Pairwise comparisons between optimizers
    \item \textbf{Effect size}: Cohen's d for magnitude of differences
\end{itemize}

All statistical tests used $\alpha$ = 0.05 significance level with Bonferroni correction for multiple comparisons.
...
Table \ref{tab:statistical_significance} presents results from Friedman tests examining overall differences between optimizers within each dataset-metric combination.

\begin{table}[htbp]
\centering
\caption{Statistical Significance Summary (Friedman Tests)}
\label{tab:statistical_significance}
\begin{tabular}{@{}llrlcc@{}}
\toprule
Dataset & Metric & Groups & Friedman $\chi^2$ & p-value & Significant \\
\midrule
MNIST & Accuracy & 8 & 24.306 & 0.0010 & Yes \\
MNIST & Emissions & 8 & 6.622 & 0.4692 & No \\
CIFAR-10 & Accuracy & 8 & 46.609 & <0.0001 & Yes \\
CIFAR-10 & Emissions & 8 & 17.289 & 0.0156 & Yes \\
CIFAR-100 & Accuracy & 8 & 57.186 & <0.0001 & Yes \\
CIFAR-100 & Emissions & 8 & 44.022 & <0.0001 & Yes \\
\bottomrule
\end{tabular}
\end{table}

The statistical analysis confirms significant differences between optimizers for accuracy across all datasets. For emissions, differences become statistically significant as dataset complexity increases, suggesting that optimizer choice has more pronounced environmental implications for challenging problems.
